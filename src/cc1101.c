...
#define cc1101_Select()  gpio_set_level(CONFIG_C1101_CSN_GPIO, LOW)
// Deselect (SPI) CC1101
//#define cc1101_Deselect()  digitalWrite(SS, HIGH)
#define cc1101_Deselect()  gpio_set_level(CONFIG_C1101_CSN_GPIO, HIGH)
// Wait until SPI MISO line goes low
//#define wait_Miso()  while(digitalRead(MISO)>0)
#define wait_Miso()  while(gpio_get_level(CONFIG_C1101_MISO_GPIO)>0)
//#define wait_Miso()  (void)0
// Get GDO0 pin state
//#define getGDO0state()	digitalRead(GDO0)
#define getGDO0state()	gpio_get_level(CONFIG_C1101_GDO0_GPIO)
...
void spi_init() {
	gpio_reset_pin(CONFIG_C1101_CSN_GPIO);
	gpio_set_direction(CONFIG_C1101_CSN_GPIO, GPIO_MODE_OUTPUT);
	gpio_set_level(CONFIG_C1101_CSN_GPIO, 1);

	spi_bus_config_t buscfg = {
		.sclk_io_num = CONFIG_C1101_SCK_GPIO, // set SPI CLK pin
		.mosi_io_num = CONFIG_C1101_MOSI_GPIO, // set SPI MOSI pin
		.miso_io_num = CONFIG_C1101_MISO_GPIO, // set SPI MISO pin
		.quadwp_io_num = -1,
		.quadhd_io_num = -1
	};
...
esp_err_t init(uint8_t freq, uint8_t mode)
{
	...
	io_conf.pin_bit_mask = 1ULL<<CONFIG_C1101_GDO0_GPIO;
	//set as input mode
	...
	gpio_isr_handler_add(CONFIG_C1101_GDO0_GPIO, gpio_isr_handler, (void*) CONFIG_C1101_GDO0_GPIO);
